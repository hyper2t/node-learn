...
export async function exportData(userId: string): Promise<Record<string, unknown>> {
  ...
  return { exportedAt: new Date().toISOString(), profile, roles, studentProfile: student, teacherProfile: teacher, requests: [...requestsS, ...requestsT], relations: [...relationsS, ...relationsT], evidence, evidenceRevisions, learningSummaries: [...summariesS, ...summariesT], feedback, messagesSent: messages, contacts, blocks };
}
