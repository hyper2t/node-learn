...
export async function exportData(userId: string): Promise<Record<string, unknown>> {
  ...
  const [profile, roles, student, teacher, requestsS, requestsT, relationsS, relationsT, evidence, feedback, contacts, blocks, evidenceRevisions, summariesS, summariesT, reviewsW, reviewsR] = await Promise.all([
    ...
    by(TABLES.evidenceRevisions, 'authorId'), by(TABLES.relationSummaries, 'studentId'), by(TABLES.relationSummaries, 'teacherId'),
    by(TABLES.teacherReviews, 'studentId'), by(TABLES.teacherReviews, 'teacherId'),
  ]);
  const messages = await by(TABLES.messages, 'senderId');
  return { exportedAt: new Date().toISOString(), profile, roles, studentProfile: student, teacherProfile: teacher, requests: [...requestsS, ...requestsT], relations: [...relationsS, ...relationsT], evidence, evidenceRevisions, learningSummaries: [...summariesS, ...summariesT], reviewsWritten: reviewsW, reviewsReceived: reviewsR, feedback, messagesSent: messages, contacts, blocks };
}
